    ...
    While 1
        Switch GUIGetMsg()
			...
			Case $idButton_prtSc
			   ...
			   GUISetState(@SW_SHOW, $hGUI)
			   Run("mspaint.exe")
			   WinWait($paintNm & ' - Paint')
			   WinActivate($paintNm & ' - Paint')
			   WinSetState($paintNm & ' - Paint', "", @SW_MAXIMIZE)
			   Send("^v")
...
Func snSt($ptSh, $fName)
   ...
   Run("mspaint.exe")
   WinWait($paintNm & ' - Paint', "", 10)
   WinActivate($paintNm & ' - Paint')
   WinSetState($paintNm & ' - Paint', "", @SW_MAXIMIZE)
   Send("^v")
   WinClose($paintNm & ' - Paint')
   WinWait("Paint", "", 10)
   WinActive("Paint")
   WinWaitActive("Paint", "", 10)
   Send("{ENTER}")
   ...
   ControlSetText("Save As","","Edit1", @ScriptDir & "\CSM(" & $fCnt & ")\" & $fName & ".png")

   if _OSVersion() == 6.1 Then
	  Send("{RIGHT}")
	  Send("{SPACE}")
	  Send("{BS}")
   EndIf
